...
def validate(df, table_name):
    ...
    for csv_col in df.columns:
        if csv_col not in table_col_map and csv_col.lower() in table_col_lower:
            ...
            warnings.append(
                f"Column '{csv_col}' auto-renamed to '{correct}' to match table schema."
            )
    if rename_map:
    ...
    for col in df.columns:
        ...
        if df[col].isnull().all():
            warnings.append(
                f"Column '{col}' has no values — all rows are empty for this column."
            )
    ...
    for col in table_cols:
        ...
        if not col.get("nullable", True):
            ...
            if n:
                ...
                errors.append(
                    f"Column '{name}' has {n} null value(s) in rows: {bad_rows}"
                )
    ...
    for col in table_cols:
        ...
        if isinstance(col_type, (Integer, BigInteger, Float, Numeric)):
            ...
            if bad_rows:
                errors.append(
                    f"Column '{name}' has non-numeric values in rows: {bad_rows}"
                )
            ...
            if bad_rows:
                errors.append(
                    f"Column '{name}' has invalid date/datetime values in rows: {bad_rows}"
                )
            else:
                ...
                if not_iso:
                    ...
                    warnings.append(
                        f"Column '{name}' date format auto-converted to YYYY-MM-DD HH:MM:SS."
                    )
    ...
    for col in table_cols:
        ...
        if isinstance(col_type, (String, CHAR)):
            ...
            if series.str.len().max() > max_len:
                bad_rows = df[
                    df[name].notna() & (df[name].astype(str).str.len() > max_len)
                ].index.tolist()
                errors.append(
                ...
                logger.error(
                    "Column '%s' exceeds max length %d in rows: %s",
                    name,
                    max_len,
                    bad_rows,
                )
    ...
    if len(df) > 5000:
        warnings.append(
            f"Large file: {len(df)} rows detected. Please confirm before inserting."
        )

    # 8. Duplicate PK check
    pk_info = inspector.get_pk_constraint(table_name)
    pk_cols = [
        c for c in pk_info.get("constrained_columns", []) if c not in _AUTO_COLS
    ]
    ...
    # 9. Fully duplicate rows within CSV
    ...
    # Also check partial duplicates across all data columns
    non_auto_cols = [c for c in df.columns if c not in _AUTO_COLS]
    if len(non_auto_cols) > 1:
        partial_dupe_mask = df.duplicated(subset=non_auto_cols, keep="first")
        partial_count = int(partial_dupe_mask.sum())
        if partial_count > 0 and partial_count != int(full_dupe_mask.sum()):
            warnings.append(
                f"Found {partial_count} rows with duplicate values across all "
                f"data columns. Review before inserting."
            )

    # 10. Rows already present in DB (only if column check passed)
    if column_check_passed:
        ...
        try:
            ...
            if not existing_df.empty:
                ...
                df_str = df[check_cols].astype(str)
                existing_str = existing_df[check_cols].astype(str)
                merged = df_str.merge(existing_str, on=check_cols, how="inner")
                if not merged.empty:
                    n = len(merged)
